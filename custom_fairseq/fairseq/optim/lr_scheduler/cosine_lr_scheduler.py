...
class CosineLRSchedule(FairseqLRScheduler):
    ...
    def reinit(self, total_num_update, num_updates):
        ...
        self.period = self.total_num_update = total_num_update

        # only enable below when set warmup_ratio
        if self.cfg.warmup_ratio <= 0:
            return

        self.warmup_updates = self.cfg.warmup_updates = int(self.total_num_update * self.cfg.warmup_ratio)
        self.lr_step = (self.max_lr - self.cfg.warmup_init_lr) / self.cfg.warmup_updates

        warmup_factor = min(1.0, num_updates / float(self.warmup_updates))
